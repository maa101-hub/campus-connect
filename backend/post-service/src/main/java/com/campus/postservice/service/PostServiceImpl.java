...
public class PostServiceImpl implements PostService {
    ...
    public PagedResponse<PostResponse> getFeed(Long currentUserId, int page, int size) {
        log.info("Fetching paginated feed page={}, size={}", page, size);
        Pageable pageable = PageRequest.of(page, size, Sort.by("createdAt").descending());
        Page<Post> postPage = postRepository.findByActiveTrue(pageable);
        return mapToPagedResponse(postPage, currentUserId);
    }
    ...
    public void addComment(Long postId, AddCommentRequest request) {
        ...
        log.info("Comment added successfully on postId: {}", postId);
    }
    ...
    public void deletePost(Long postId) {
        ...
        Post post = postRepository.findById(postId).orElseThrow(() -> new RuntimeException("Post not found"));
        if (!post.getActive()) throw new RuntimeException("Post already deleted");
    ...
    public void updatePost(Long postId, UpdatePostRequest request) {
		...
		Post post = postRepository.findById(postId).orElseThrow(() -> new RuntimeException("Post not found"));
		if (!post.getActive()) throw new RuntimeException("Cannot update deleted post");
		post.setContent(request.getContent());
		postRepository.save(post);
    ...
    public String toggleLike(Long postId, Long userId) {
        log.info("Like toggle request postId={}, userId={}", postId, userId);
        Post post = postRepository.findById(postId).orElseThrow(() -> new RuntimeException("Post not found"));
    ...
    @Override
    public PagedResponse<PostResponse> getCollegeFeed(Long currentUserId, String collegeName, int page, int size) {
        log.info("Fetching college feed for: {}", collegeName);
        Pageable pageable = PageRequest.of(page, size, Sort.by("createdAt").descending());
        Page<Post> postPage = postRepository.findByActiveTrueAndCollegeNameOrderByCreatedAtDesc(collegeName, pageable);
        return mapToPagedResponse(postPage, currentUserId);
    }

    @Override
    public PagedResponse<PostResponse> getUserPosts(Long currentUserId, Long targetUserId, int page, int size) {
        log.info("Fetching posts for user: {}", targetUserId);
        Pageable pageable = PageRequest.of(page, size, Sort.by("createdAt").descending());
        Page<Post> postPage = postRepository.findByActiveTrueAndUserIdOrderByCreatedAtDesc(targetUserId, pageable);
        return mapToPagedResponse(postPage, currentUserId);
    }

    private PagedResponse<PostResponse> mapToPagedResponse(Page<Post> postPage, Long currentUserId) {
        ...
        for (Post post : postPage.getContent()) {
            ...
            if (currentUserId != null) {
                dto.setHasLiked(postLikeRepository.findByPostIdAndUserId(post.getId(), currentUserId).isPresent());
            } else {
